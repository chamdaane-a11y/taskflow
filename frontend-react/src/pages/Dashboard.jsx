...
import {
  ...
  ChevronDown, ChevronUp, ExternalLink, Slack
} from 'lucide-react'
...
export default function Dashboard() {
  ...
  const [erreurForm, setErreurForm] = useState('') // Pour la consigne 3
  ...
  // useEffect pour l'installation PWA - ajouté ici
  useEffect(() => {
    const handler = (e) => {
      e.preventDefault()
      setInstallPrompt(e)
  ...
  const chargerSlackWebhook = async () => {
    ...
    } catch (err) { /* pas encore configuré */ }
  }
  ...
  const ajouterTache = async () => {
    ...
    // Consigne 3 : validation date+heure obligatoire
    if (!deadline) { 
      setErreurForm("La date et l'heure sont obligatoires."); 
      return 
    }
    
    await axios.post(`${API}/taches`, { titre, priorite, deadline: deadline.toISOString().slice(0, 16), user_id: user.id })
    setTitre(''); setDeadline(null)
    setErreurForm('')
  ...
  const toggleExpand = (id) => setExpandedTaches(prev => ({ ...prev, [id]: !prev[id] }))
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: T.bg, color: T.text, transition: 'all 0.5s ease', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <AnimatePresence>
        {notification && (
          <motion.div style={{ position: 'fixed', top: 'clamp(16px, 4vh, 24px)', right: 'clamp(16px, 4vw, 24px)', zIndex: 1000, maxWidth: 'min(400px, 90vw)', background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 12, padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 10, boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}
      ...
      <aside style={{ width: 'min(248px, 80%)', maxWidth: '248px', background: T.bg2, borderRight: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', padding: 'clamp(16px, 3vh, 24px) clamp(12px, 2vw, 16px)', position: 'fixed', top: 0, left: isMobile ? (showSidebar ? 0 : '-100%') : 0, height: '100vh', transition: 'left 0.3s ease', zIndex: 100, overflowY: 'auto', paddingBottom: '80px' }}>
        ...
        <nav style={{ flex: 1 }}>
          ...
          {[{ val: 'toutes', label: 'Toutes les tâches' }, { val: 'haute', label: 'Priorité haute' }, { val: 'terminee', label: 'Terminées' }].map(f => (
                  ...
                  <motion.div style={{ background: T.bg2, borderRadius: 8, padding: 10, marginBottom: 8, border: `1px solid ${T.border}` }}
                    ...
                    <div style={{ marginBottom: 10 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                        ...
                        {slackWebhook && <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#4caf82', marginLeft: 'auto' }} title="Configuré" />}
                      </div>
                      <input
                        style={{ width: '100%', padding: '6px 8px', background: T.bg3, border: `1px solid ${T.border}`, borderRadius: 7, color: T.text, fontSize: 11, outline: 'none', boxSizing: 'border-box', marginBottom: 6 }}
                      ...
                      />
                      <motion.button
                        style={{ width: '100%', padding: '6px 10px', background: slackSaved ? '#4caf82' : `${T.accent}15`, border: `1px solid ${slackSaved ? '#4caf82' : T.accent + '40'}`, borderRadius: 7, color: slackSaved ? 'white' : T.accent, fontSize: 11, fontWeight: 600, cursor: slackSaving ? 'not-allowed' : 'pointer' }}
...
<AnimatePresence>
  {showInstallBanner && !appInstalled && (
    <motion.div
      style={{ background: `linear-gradient(135deg, #6c63ff20, #63b3ed15)`, border: `1px solid #6c63ff40`, borderRadius: 14, padding: '14px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}
      initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        {/* Stats */}
        <div className="stats-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 'clamp(8px, 2vw, 12px)', marginBottom: 24 }}>
          {[
        ...
        <div className="forms-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'clamp(12px, 2vw, 16px)', marginBottom: 24 }}>
          <motion.div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 'clamp(12px, 3vw, 20px)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            ...
            {/* Consigne 2 : remplacement du select par PrioriteSelect */}
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <PrioriteSelect value={priorite} onChange={setPriorite} T={T} />
              
              {/* Consigne 3 : DatePicker avec heure */}
              <DatePicker
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: 16, flexWrap: 'wrap' }}>
          {[['toutes', 'Toutes'], ['haute', 'Haute'], ['moyenne', 'Moyenne'], ['basse', 'Basse'], ['terminee', 'Terminées']].map(([val, label]) => (
            <motion.button key={val}
              style={{ padding: '5px 14px', background: filtre === val ? `${T.accent}15` : 'transparent', border: `1px solid ${filtre === val ? T.accent : T.border}`, borderRadius: 99, color: filtre === val ? T.accent : T.text2, fontSize: 12, fontWeight: filtre === val ? 600 : 400, cursor: 'pointer' }}
              onClick={() => setFiltre(val)} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>{label}</motion.button>
          ))}
        </div>
            ...
            {tachesFiltrees.map((tache, i) => {
              ...
              return (
                <motion.div key={tache.id}
                  style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 12, padding: 'clamp(10px, 2vw, 14px)', marginBottom: 8, opacity: tache.terminee ? 0.5 : 1 }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                    <motion.button
                      ...
                      onClick={() => toggleTache(tache.id, tache.terminee, tache.priorite)} whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }}>
                      {tache.terminee && <CheckSquare size={10} color="white" strokeWidth={3} />}
                    </motion.button>

                    <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, textDecoration: tache.terminee ? 'line-through' : 'none', color: tache.terminee ? T.text2 : T.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{tache.titre}</div>
                    ...
                    {/* ===== BOUTON GOOGLE CALENDAR (consigne 1) ===== */}
                    ...
                    {/* Bouton sous-tâches */}
                    <motion.button
                      style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', background: isExpanded ? `${T.accent}15` : 'transparent', border: `1px solid ${isExpanded ? T.accent : T.border}`, borderRadius: 8, color: isExpanded ? T.accent : T.text2, fontSize: 11, cursor: 'pointer', flexShrink: 0 }}
                      onClick={() => toggleExpand(tache.id)} whileHover={{ borderColor: T.accent, color: T.accent }}>
                      {isExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                      <span style={{ display: isMobile ? 'none' : 'inline' }}>Sous-tâches</span>
                    </motion.button>

                    <motion.button
                      style={{ padding: '4px 10px', background: 'transparent', border: `1px solid ${T.border}`, color: T.text2, borderRadius: 8, fontSize: 12, cursor: 'pointer', flexShrink: 0 }}
                      onClick={() => toggleTache(tache.id, tache.terminee, tache.priorite)} whileHover={{ borderColor: '#4caf82', color: '#4caf82' }}>

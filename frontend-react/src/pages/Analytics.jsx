...
export default function Analytics() {
  ...
  const [periode, setPeriode] = useState('7jours') // '7jours' | '30jours'
  ...
  // 1. Derniers 7 jours (pour comparaison)
  ...
  // 2. Jours de la semaine dernière (pour comparaison)
  const semaineDerniereLabels = Array.from({ length: 7 }, (_, i) => {
    const d = new Date()
  ...
  const tachesSemaineDerniere = semaineDerniereKeys.map(jour => {
    const found = stats?.par_jour_semaine_derniere?.find(p => p.jour?.split('T')[0] === jour || p.jour === jour)
    return found ? found.count : 0
  })

  // 4. Données pour heatmap des heures
  const heuresLabels = Array.from({ length: 24 }, (_, i) => `${i}h`)
  const heuresData = stats?.par_heure || Array(24).fill(0)
  
  // 5. Moyennes et analyses
  ...
  // 6. Projection fin de semaine
  const joursRestants = 7 - new Date().getDay() // simplifié
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: T.bg, color: T.text, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <main style={{ 
        ...
        <div className="kpi-grid" style={{ 
          ...
          marginBottom: 'clamp(24px, 4vh, 28px)' 
        }}>
          {[
            {
              icon: Target,
            ...
            },
            {
              icon: CheckSquare,
              label: 'Moyenne / jour',
              value: moyenneJour.toFixed(1),
              sub: `${stats?.cette_semaine || 0} cette semaine`,
              color: '#4caf82'
            },
            {
              icon: Zap,
              label: 'Meilleur jour',
              value: meilleurJourValeur,
              sub: meilleurJour,
              color: '#e08a3c'
            },
            {
        ...
        <div className="charts-grid" style={{ 
          ...
          {/* COMPARAISON JOUR PAR JOUR - NOUVEAU */}
          <motion.div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 'clamp(16px, 3vw, 24px)' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 'clamp(13px, 2.5vw, 14px)', fontWeight: 600, color: T.text }}>Comparaison jour par jour</p>
              <p style={{ fontSize: 'clamp(11px, 2.5vw, 12px)', color: T.text2, marginTop: 2 }}>Cette semaine vs semaine dernière</p>
            </div>
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    legend: {
                      display: true,
                      position: 'top',
                      labels: { color: T.text2, font: { size: 11 }, boxWidth: 12, padding: 8 }
                    }
                  }
                }}
              />
            ...
            <div style={{ marginTop: 16, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              {joursLabels.map((jour, idx) => {
                ...
                return (
                  <div key={jour} style={{ fontSize: 11, color: diff > 0 ? '#4caf82' : '#e05c5c', background: diff > 0 ? '#4caf8215' : '#e05c5c15', padding: '4px 10px', borderRadius: 99 }}>
                    {jour}: {diff > 0 ? '+' : ''}{diff} tâche{diff !== 1 ? 's' : ''}
                  </div>
                )
              })}
            </div>
          ...
          {/* HEATMAP DES HEURES - NOUVEAU */}
          <motion.div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 'clamp(16px, 3vw, 24px)' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.35 }}>
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 'clamp(13px, 2.5vw, 14px)', fontWeight: 600, color: T.text }}>Moment de productivité</p>
              <p style={{ fontSize: 'clamp(11px, 2.5vw, 12px)', color: T.text2, marginTop: 2 }}>Tâches complétées par heure</p>
            </div>
            <div className="chart-container" style={{ height: isMobile ? 250 : 300, position: 'relative' }}>
              <Bar
                data={{
                  labels: heuresLabels.filter((_, i) => i % 3 === 0), // Toutes les 3h pour lisibilité
                  datasets: [{
                    data: heuresData.filter((_, i) => i % 3 === 0),
                    backgroundColor: (ctx) => {
                      const value = ctx.dataset.data[ctx.dataIndex]
                      const max = Math.max(...heuresData)
                      const intensity = value / max
                      return `rgba(108, 99, 255, ${0.3 + intensity * 0.5})`
                    },
                    borderRadius: 8,
                  }]
                }}
            ...
            {/* Heure de pic */}
            {(() => {
              const maxHeure = heuresData.indexOf(Math.max(...heuresData))
              if (maxHeure >= 0) {
                return (
                  <div style={{ marginTop: 16, textAlign: 'center', background: `${T.accent}10`, padding: '10px', borderRadius: 10 }}>
                    <p style={{ fontSize: 12, color: T.text2 }}>Votre moment le plus productif est</p>
                    <p style={{ fontSize: 18, fontWeight: 700, color: T.accent }}>{maxHeure}h - {maxHeure + 1}h</p>
                  </div>
                )
              }
              return null
        ...
        <div className="charts-grid" style={{ 
          ...
          {/* Tâches par jour (courbe d'évolution) */}
          <motion.div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 'clamp(16px, 3vw, 24px)' }}
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 'clamp(13px, 2.5vw, 14px)', fontWeight: 600, color: T.text }}>Évolution quotidienne</p>
              <p style={{ fontSize: 'clamp(11px, 2.5vw, 12px)', color: T.text2, marginTop: 2 }}>Tendance sur les 7 derniers jours</p>
            </div>
            <div className="chart-container" style={{ height: isMobile ? 250 : 300, position: 'relative' }}>
              <Line
                data={{
          ...
          <motion.div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 'clamp(16px, 3vw, 24px)' }}
            ...
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 'clamp(13px, 2.5vw, 14px)', fontWeight: 600, color: T.text }}>Répartition par priorité</p>
              <p style={{ fontSize: 'clamp(11px, 2.5vw, 12px)', color: T.text2, marginTop: 2 }}>Distribution de vos tâches</p>
            </div>
            <div className="chart-container" style={{ height: isMobile ? 250 : 300, position: 'relative' }}>
              <Doughnut
                data={{
                ...
                options={{
                  ...
                  plugins: {
                    legend: {
                      position: 'bottom',
                      labels: { color: T.text2, font: { size: isMobile ? 10 : 11 }, padding: 16, boxWidth: 10 }
                    },
                    tooltip: {
                      backgroundColor: T.bg2,
                      titleColor: T.text,
        ...
        <motion.div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: 'clamp(16px, 3vw, 24px)', marginTop: 20 }}
          initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          <div style={{ marginBottom: 20 }}>
            <p style={{ fontSize: 'clamp(13px, 2.5vw, 14px)', fontWeight: 600, color: T.text }}>💡 Analyse personnalisée</p>
            <p style={{ fontSize: 'clamp(11px, 2.5vw, 12px)', color: T.text2, marginTop: 2 }}>Basé sur vos données des 7 derniers jours</p>
          </div>
          
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 16 }}>
            
            {/* Conseil 1 : Meilleur jour */}
            <div style={{ background: T.bg3, borderRadius: 12, padding: 16 }}>
              ...
              {meilleurJour && (
                <p style={{ fontSize: 11, color: T.text2, marginTop: 8, fontStyle: 'italic' }}>
                  Vous êtes {meilleurJourValeur > moyenneJour * 1.5 ? "très" : "plus"} productif ce jour-là
                </p>
              )}
            </div>

            {/* Conseil 2 : Moment idéal */}
            <div style={{ background: T.bg3, borderRadius: 12, padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              {(() => {
                ...
                if (maxHeure >= 0) {
                  return (
                    <>
                      <p style={{ fontSize: 24, fontWeight: 700, color: '#e08a3c', marginBottom: 4 }}>{maxHeure}h - {maxHeure + 1}h</p>
                      <p style={{ fontSize: 12, color: T.text2 }}>Votre pic de concentration</p>
                      <p style={{ fontSize: 11, color: T.text2, marginTop: 8 }}>
                        Bloquez ce créneau pour les tâches importantes
                      </p>
                    </>
                  )
                }
                return <p style={{ fontSize: 12, color: T.text2 }}>Pas assez de données</p>
              })()}
            </div>

            {/* Conseil 3 : Progression */}
            <div style={{ background: T.bg3, borderRadius: 12, padding: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <TrendingUp size={20} color={evolutionPositive ? '#4caf82' : '#e05c5c'} />
                <span style={{ fontSize: 13, fontWeight: 600, color: T.text }}>Tendance</span>
              </div>
              <p style={{ fontSize: 24, fontWeight: 700, color: evolutionPositive ? '#4caf82' : '#e05c5c', marginBottom: 4 }}>
                {evolutionPositive ? '+' : ''}{evolution}%
              </p>
              <p style={{ fontSize: 12, color: T.text2 }}>
                {evolutionPositive 
                  ? "Vous êtes plus productif que la semaine dernière" 
                  : "Vous êtes moins productif que la semaine dernière"}
              </p>
              {!evolutionPositive && (
                <p style={{ fontSize: 11, color: T.text2, marginTop: 8 }}>
                  Essayez de vous concentrer sur {pireJour || "vos jours creux"}
                </p>
              )}
            </div>
          </div>

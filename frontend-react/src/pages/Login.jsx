...
function LoginInner() {
  ...
  const loginTotp = async () => {
    ...
    try {
      ...
      navigate(pendingInviteCode ? `/collaboration?code=${encodeURIComponent(pendingInviteCode)}` : '/dashboard')
    } catch (err) {
          ...
          <AnimatePresence mode="wait">
            {twoFaRequired ? (
              <motion.div key="2fa" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                ...
                {erreur && (
                  <div style={{ background: 'var(--danger-soft)', border: '1px solid var(--danger)', borderRadius: 10, padding: '11px 14px', fontSize: 13, color: 'var(--danger)', marginBottom: 14 }}>
                    {erreur}
                  </div>
                )}
